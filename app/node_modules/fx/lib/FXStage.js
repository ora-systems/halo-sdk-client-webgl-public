...
var viewportStack = [];
var viewportCurrent = [0, 0, 0, 0];

glu.viewport = function(x, y, w, h) {
  var gl = Context.currentContext;
  var curr = viewportCurrent;

  if (curr[0] != x || curr[1] != y || curr[2] != w || curr[3] != h) {
...
glu.pushViewport = function() {
  var viewportCopy = [viewportCurrent[0], viewportCurrent[1], viewportCurrent[2], viewportCurrent[3]];
  viewportStack.push(viewportCopy);
}

glu.popViewport = function() {
  if (viewportStack.length > 0) {
    var prev = viewportCurrent;
    var curr = viewportStack.shift();
    if (curr[0] != prev[0] || curr[0] != prev[0] || curr[0] != prev[0] || curr[0] != prev[0]) {
      gl.viewport(viewportCurrent[0], viewportCurrent[1], viewportCurrent[2], viewportCurrent[3]);
    }
    viewportCurrent = curr;
  }
  else {
    throw new Error('Viewport stack is empty!');
  }
}
...
FXStage.prototype.getOutputSize = function(width, height, verbose) {
  if (width && height) {
    ...
    throw new Error('FXStage is missing width and height')
