...
void main() {
  ...
  float frequency = remap(complexity, 0.0, 1.0, 0.1, 0.8);
  //float amplitude = (1.0 - (1.0 - radiusHeightLocalScale) * (1.0 - radiusHeightLocalScale)) / 2.0; //remaping 0..1 to 3..14 radius of original halo
  float amplitude = radiusHeightLocalScale * radiusHeightLocalScale / 2.0; //remaping 0..1 to 3..14 radius of original halo

  float horizontalFrequency = complexity * 1.2;
  float horizontalAmplitude = radiusHeightLocalScale * complexity * 1.2; //remaping 0..1 to 3..14 radius of original halo
  ...
  float y = amplitude * ( 0.5 + 0.5 * cnoise2( noiseDir ));
  ...
  x += horizontalOffsetAmplitude * horizontalAmplitude * ( perlin1 ) * sinValue;
  z += horizontalOffsetAmplitude * horizontalAmplitude * ( perlin1 ) * cosValue;

  x += horizontalOffsetAmplitude2 * horizontalAmplitude * ( perlin2 ) * sinValue;
  z += horizontalOffsetAmplitude2 * horizontalAmplitude * ( perlin2 ) * cosValue;

  //wobble
  x += wobbleAmplitude * ( perlin3 ) * sinValue;
  z += wobbleAmplitude * ( perlin3 ) * cosValue;
