...
void main() {
  ...
  if (!stratified) {
    ...
    float slowdown = 5.0;
    //float offset = (t + time * speed / slowdown) * floor(minCycles + complexity * (maxCycles - minCycles));
    float offset = (t) * floor(minCycles + complexity * (maxCycles - minCycles));

    float funCos = cos(offset * 2.0 * PI);
    float funTri = 4.0 * abs(fract(offset) - 0.5) - 1.0;
    float c = smoothstep(0.75, 0.85, complexity);
    float fun = mix(funCos, funTri, c);
    fun *= sin(time * speed * 5.0);
    y = complexity * stratifiedAmplitude * fun;
  }
...
void main() {
  ...
  vec4 lineColor  = texture2D(colorTexture, vec2(color * 0.99, 0.5));

  gl_FragColor = linePattern * lineColor;

  gl_FragColor.rgb = pow(gl_FragColor.rgb, vec3(2.2));
  gl_FragColor *= alpha * brightness;
  gl_FragColor *= (1.0 - vTexCoord.y)*(1.0 - vTexCoord.y);
