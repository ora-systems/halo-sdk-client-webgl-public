...
Halo.prototype.buildMesh = function(opts) {
  var points    = makeCircle(1, 64, 'x', 'z', true);
  ...
  var lineTexture = Texture2D.load(opts.lineTexture, { repeat: true, mipmap: true });
  var colorTexture = Texture2D.load(opts.colorTexture, { repeat: false, mipmap: true });
  var mat = new HaloRingMaterial({ lineTexture: lineTexture, colorTexture: colorTexture } );
...
Halo.prototype.draw = function(camera) {
  this.update();
  this.ringMesh.material.uniforms.radius = 1.0;
  //this.ringMesh.draw(camera, this.ringInstances);
