...
function Halo(ctx, window, opts) {
  ...
  this.grid = [0.5, 0.5, 0.5, 1];
  this.spectrum = [[1,0,0,1], [0,0,1,1]];
  this.growth = 0.05;
  this.glow = 0.75;
  this.scale = 40;
...
Halo.prototype.buildMesh = function(opts) {
  ...
  var lineDotsTexture = this.lineDotsTexture = ctx.createTexture2D(null, 16, 1, { repeat: true, mipmap: true });
  io.loadImage(opts.lineDotsTexture, function(err, img) {
    lineDotsTexture.update(img);
    //ctx.bindTexture(lineDotsTexture);
    //var gl = ctx.getGL();
    //gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MIN_FILTER, gl.NEAREST);
    //gl.generateMipmap(gl.TEXTURE_2D);
  })
  var lineSolidTexture = this.lineSolidTexture = ctx.createTexture2D(null, 16, 1, { repeat: true, mipmap: true });
  io.loadImage(opts.lineSolidTexture, function(err, img) {
    lineSolidTexture.update(img);
  })

  var colorTexture = this.colorTexture = ctx.createTexture2D(null, 256, 32);
  io.loadImage(opts.colorTexture, function(err, img) {
    colorTexture.update(img);
  })
  ...
  this.defaultRingUniforms = { lineTexture: lineDotsTexture, colorTexture: colorSpectrumTexture }
...
Halo.prototype.drawSolid = function(camera) {
  ...
  ctx.setClearColor(this.backgroundTransparent[0], this.backgroundTransparent[1], this.backgroundTransparent[2], this.backgroundTransparent[3]);
