...
Halo.prototype.updateWaves = function() {
    ...
    this.waveInstances.forEach(function(wave) {
        ...
        wave.uniforms.wobble = this.wobble;
        wave.uniforms.wobbleFadeout = this.wobbleFadeout;
    }.bind(this))
...
Halo.prototype.animate = function() {
  for(var i=0; i<this.ringInstances.length; i++) {
    ...
    ring.uniforms.radiusHeightScale = lerp(minHeightFadeout, maxHeightFadeout, this.coreSmoothing)
    ring.uniforms.radiusHeightLocalScale = clamp(remap(i/this.maxNumRings, 0, this.size, 0, 1), 0, 1);
    ...
    if (this.highlightRing) {
        var s = i / this.maxNumRings;
        var ns = (i + 1) / this.maxNumRings;
        if (s <= this.highlightRing && this.highlightRing < ns) {
            ring.uniforms.highlight = 3 * this.window.getPixelRatio();
        }
    }
...
Halo.prototype.draw = function(camera, camera2D, w, h) {
  ...
  ctx.setLineWidth(1 * this.window.getPixelRatio());
  ...
  ctx.setLineWidth(2 * this.window.getPixelRatio());
  if (this.showGrid) {
