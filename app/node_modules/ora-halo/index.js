...
var HALO_RING_VERT = glslify(__dirname + '/assets/Basic.vert')
var HALO_RING_FRAG = glslify(__dirname + '/assets/Basic.frag')
...
Halo.prototype.buildMesh = function(opts) {
  ...
  var lineDotsTexture = this.lineDotsTexture = ctx.createTexture2D(null, 16, 1); //TODO: Texture2D.load(opts.lineDotsTexture, { repeat: true, mipmap: true });
  var lineSolidTexture = this.lineSolidTexture = ctx.createTexture2D(null, 16, 1); //TODO: Texture2D.load(opts.lineSolidTexture, { repeat: true, mipmap: true });

  var colorTexture = this.colorTexture = ctx.createTexture2D(null, 256, 32); //TODO: this.colorSpectrumTexture.update(createGradient(this.spectrum, 256, 32));
  var gridColorTexture = this.gridColorTexture = ctx.createTexture2D(null, 256, 32); //TODO: var gridColorTexture = this.gridColorTexture = Texture2D.load(opts.gridColorTexture, { repeat: false, mipmap: true });
...
Halo.prototype.draw = function(camera, camera2D, w, h) {
  //TODO: if (!this.ringMesh.material.program.ready) return;
  ...
  //TODO: this.material.uniforms.lineTexture = this.lineDotsTexture;

  this.drawMeshInstances(ctx, camera, this.ringProgram, this.ringInstances, this.defaultRingUniforms)
  this.drawMeshInstances(ctx, camera, this.ringProgram, this.gridLineInstances, this.defaultGridUniforms)
  ...
  //TODO: this.gridLineLabels.forEach(function(label, i) {
    ...
    //label.draw(camera2D);
  //}.bind(this));
  ctx.popState(ctx.COLOR_BIT | ctx.DEPTH_BIT | ctx.BLEND_BIT);
}
