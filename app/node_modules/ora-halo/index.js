...
//TODO: var HaloRingMaterial = require('ora-halo-ring-material');
...
function Halo(ctx, window, opts) {
  ...
  this.grid = [1,0,0,1];
  this.gradient = [[1,0,0,1], [1,0,0,1]];
  this.growth = 0.05;
  this.glow = 0.75;
  this.scale = 40;
  this.spectrum = ['222222'];
...
Halo.prototype.buildMesh = function(opts) {
  var ctx = this.ctx;
  //TODO: var lineDotsTexture = this.lineDotsTexture = Texture2D.load(opts.lineDotsTexture, { repeat: true, mipmap: true });
  //TODO: var lineSolidTexture = this.lineSolidTexture = Texture2D.load(opts.lineSolidTexture, { repeat: true, mipmap: true });
  //TODO: var colorTexture = this.colorTexture = Texture2D.load(opts.colorTexture, { repeat: false, mipmap: true });
  this.colorSpectrumTexture = ctx.createTexture2D(null, 256, 32);
  //TODO: this.colorSpectrumTexture.update(createGradient(this.spectrum, 256, 32));
  //TODO: this.material = new HaloRingMaterial({ lineTexture: lineDotsTexture, colorTexture: colorTexture } );
  //TODO: var gridColorTexture = this.gridColorTexture = Texture2D.load(opts.gridColorTexture, { repeat: false, mipmap: true });
  //TODO: this.gridMaterial = new HaloRingMaterial({ lineTexture: lineDotsTexture, colorTexture: gridColorTexture } );
...
Halo.prototype.updateGeometry = function(mode) {
  var ctx = this.ctx;
  var numPoints = (mode == 'timeline') ? 32 : 128;
...
Halo.prototype.updateGridLines = function() {
  ...
  for(var i=1; i<=numGridLines; i++) {
    ...
    var radius = remap(k*k, 0, 1, this.minRingRadius, this.maxRingRadius*1.1);
    ...
    if (i >= 2 && this.enableLabels) {
      ...
      label.radius = radius;
      this.gridLineLabels.push(label);
    }
  }
}
...
Halo.prototype.updateParams = function() {
  ...
  for(var i=0; i<this.ringInstances.length; i++) {
    ...
    ring.uniforms.radius = remap(k*k, 0, 1, this.minRingRadius, this.maxRingRadius);
...
Halo.prototype.setColorSpectrum = function(colors) {
    ...
    console.log('setColorSpectrum', colors);
    var tex = this.colorSpectrumTexture;

    var gradient = createGradient(colors, tex.width, tex.height);
    tex.update(gradient);
    this.material.uniforms.colorTexture = this.colorSpectrumTexture;
}

Halo.prototype.draw = function(camera, camera2D, w, h) {
  if (!this.ringMesh.material.program.ready) return;
  glu.clearColor(Color.Transparent);
  ...
  //TODO: this.gridMesh = new Mesh(g, this.gridMaterial, { lines: true });
  this.ringMesh.drawInstancesOpt(camera, this.ringInstances);
  this.gridMesh.drawInstancesOpt(camera, this.gridLineInstances);
  this.gridLineLabels.forEach(function(label, i) {
  ...
  glu.enableAlphaBlending(false, false);
}

Halo.prototype.drawSolid = function(camera) {
  this.backgroundTransparent.r = this.background.r;
  this.backgroundTransparent.g = this.background.g;
  this.backgroundTransparent.b = this.background.b;
  glu.clearColor(this.backgroundTransparent);
  if (!this.ringMesh.material.program.ready) return;
